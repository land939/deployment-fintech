    ...
    with db.engine.connect() as conn:
        ...
        if "fraud_alerts" in sa_inspect(db.engine).get_table_names():
            ...
            if "email_sent" not in fa_cols:
                conn.execute(
                    text("ALTER TABLE fraud_alerts ADD COLUMN email_sent BOOLEAN DEFAULT 0")
                )
...
token_contract = w3.eth.contract(
    address=Web3.to_checksum_address(os.getenv("TOKEN_ADDRESS", "0x0")),
    abi=load_abi("FintechToken"),
)
registry_contract = w3.eth.contract(
    address=Web3.to_checksum_address(os.getenv("REGISTRY_ADDRESS", "0x0")),
    abi=load_abi("FraudRegistry"),
)
optimizer_contract = w3.eth.contract(
    address=Web3.to_checksum_address(os.getenv("OPTIMIZER_ADDRESS", "0x0")),
    abi=load_abi("TransactionOptimizer"),
)
...
def audit(event_type: str, email: str = None, success: bool = True, details: str = None):
    ...
    log_fn(
        "AUDIT | %s | email=%s | ip=%s | %s", event_type, email, request.remote_addr, details or ""
    )
...
def send_email(to_email: str, subject: str, html_body: str) -> bool:
    if not MAIL_USERNAME or not MAIL_PASSWORD:
        logger.warning(
            "MAIL_USERNAME/MAIL_PASSWORD non configurés — email non envoyé à %s", to_email
        )
        return False
...
def register():
    ...
    if not is_strong_password(password):
        return jsonify(
            {"error": "Mot de passe trop faible (min 8 caractères, 1 lettre et 1 chiffre)"}
        ), 400
    if not is_valid_wallet(wallet):
        return jsonify(
            {"error": "Adresse wallet invalide (format : 0x suivi de 40 caractères hex)"}
        ), 400
...
def login():
    ...
    if not user.check_password(password):
        ...
        audit(
            ...
            email,
            success=False,
            details=f"Tentative {user.failed_logins}/{MAX_FAILED_LOGINS}",
        )
        return jsonify({"error": "Email ou mot de passe incorrect"}), 401
...
def forgot_password():
    ...
    sent = send_email(
        email,
        "Réinitialisation de mot de passe. GTA-IT Fintech",
        build_reset_email_html(reset_link),
    )
...
def recent_transactions():
    return jsonify(
        {"transactions": list(reversed(transactions_db))[:20], "total": len(transactions_db)}
    )
...
def gas_estimate():
    try:
        ...
        return jsonify(
            {
                "recommended_gas_price": int(max(10, 50 - hour * 1.5)),
                "network_load": "45%",
                "last_updated": 0,
            }
        )
...
def superadmin_chain():
    ...
    blocks = (
        BlockchainAuditBlock.query.order_by(BlockchainAuditBlock.block_index.desc())
        .limit(limit)
        .all()
    )
    return jsonify(
...
if __name__ == "__main__":
    ...
    print(
        f"   Blockchain : {'Connecté' if w3.is_connected() else '[X] Déconnecté — Activez Ganache'}"
    )
    print("   IA Model   : Chargé")
    print(f"   Super Admin: {SUPERADMIN_EMAIL}")
    print(f"   Mode debug : {'ON' if debug_mode else 'OFF'}")
    print("   URL        : http://127.0.0.1:5000")
    print("   Admin URL  : http://127.0.0.1:5000/superadmin")
    print("=" * 55)
