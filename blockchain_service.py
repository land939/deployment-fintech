...
class BlockchainService:
    ...
    def __init__(self):
        ...
        self.token = self.w3.eth.contract(
            address=Web3.to_checksum_address(self.token_address), abi=self.token_abi
        )
        self.registry = self.w3.eth.contract(
            address=Web3.to_checksum_address(self.registry_address), abi=self.registry_abi
        )
        self.optimizer = self.w3.eth.contract(
            address=Web3.to_checksum_address(self.optimizer_address), abi=self.optimizer_abi
        )
    ...
    def mint_tokens(self, to_address: str, amount: float) -> dict:
        ...
        fn = self.token.functions.mint(Web3.to_checksum_address(to_address), amount_wei)
    ...
    def blacklist_address(self, address: str, reason: str) -> dict:
        ...
        """
        fn = self.token.functions.setBlacklist(Web3.to_checksum_address(address), True, reason)
        return self._send_transaction(fn)
    ...
    def analyze_and_report(
        ...
        if risk_level == "CRITICAL":
            bl_receipt = self.blacklist_address(
                sender_address, f"CRITICAL fraud score: {risk_score}/100 | TX: {tx_ref}"
            )
    ...
    def optimize_and_record(
        self, sender_address: str, receiver_address: str, amount: float, hour_of_day: int
    ) -> dict:
        """
        ...
        fn = self.optimizer.functions.recordOptimizedTransaction(
            Web3.to_checksum_address(receiver_address), amount_wei, estimated_gas, route_score, note
        )
        receipt = self._send_transaction(fn)
    ...
    def listen_fraud_events(self, callback):
        ...
        """
        event_filter = self.registry.events.FraudReported.create_filter(fromBlock="latest")
